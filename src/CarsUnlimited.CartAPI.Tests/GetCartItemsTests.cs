...
namespace CarsUnlimited.CartAPI.Tests
{
    ...
    public class GetCartItemsTests
    {
        ...
        public void Initialise()
        {
            ...
            _mockIRedisCacheClient.Setup(x => x.GetDbFromConfiguration().GetAsync<CartItem>("one", It.IsAny<CommandFlags>())).ReturnsAsync(new CartItem() { SessionId = "one", ItemId = "three" });
            _mockIRedisCacheClient.Setup(x => x.GetDbFromConfiguration().GetAsync<CartItem>("two", It.IsAny<CommandFlags>())).ReturnsAsync(new CartItem() { SessionId = "two", ItemId = "four" });
        }
        ...
        public async Task GivenASessionId_WhenAllItemsForSessionAreRequested_ThenAllItemsAreReturned()
        {
            ...
            Assert.AreEqual("three", result[0].ItemId);
            Assert.AreEqual("four", result[1].ItemId);
